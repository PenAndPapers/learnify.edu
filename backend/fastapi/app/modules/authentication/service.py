...
from .validation import Token, TokenAudience, TokenResponse, TokenTypeEnum, TokenRefreshRequest, TokenValidateRequest, UserToken
...
class TokenService:
  ...
  def _generate(self, audience: TokenAudience, jti: str, token_type: TokenTypeEnum) -> Token:
    """
    Generates a token
    Params:
  ...
  def validate_token(self, token: TokenValidateRequest) -> UserToken | None:
    ...
    try:
      ...
      if payload.get("type") != token.token_type:
        raise ValueError(f"Error: Token type mismatch")
      
      db_token = self.repository.get_by_token(token.token)
      
      if db_token and db_token.is_revoked:
        raise ValueError(f"Error: Token is revoked")
      
      return db_token

    except jwt.ExpiredSignatureError as e:
      error_message = f"Error: Token is expired"
      logging.error(error_message)
      raise ValueError(error_message) from e

    except jwt.InvalidTokenError as e:
      error_message = f"Error: Token signature is invalid"
      logging.error(error_message)
      raise ValueError(error_message) from e



  def refresh_token(self, token: TokenRefreshRequest, user_service: UserService) -> TokenResponse:
    """
    ...
    db_tokens = self.repository.get_by_tokens([validated_token.access_token, validated_token.refresh_token])

    # check that both access token and refresh token is in database
    if not db_tokens or len(db_tokens) != 2:
      error_message = f"Error: Unable to fetch token"
      logging.error(error_message)
      raise ValueError(error_message)
    
    access_token = next((token for token in db_tokens if token.token_type == TokenTypeEnum.ACCESS.value), None)
    refresh_token = next((token for token in db_tokens if token.token_type == TokenTypeEnum.REFRESH.value), None)

    if access_token is None or refresh_token is None:
      error_message = f"Error: Missing token"
      logging.error(error_message)
      raise ValueError(error_message)
    
    if access_token.is_revoked or refresh_token.is_revoked:
      error_message = f"Error: Attempt to use a revoked token"
      logging.error(error_message)
      raise ValueError(error_message)

    # check that both access and refresh token user_id is match
    if access_token.user_id != refresh_token.user_id:
      error_message = f"Error: Token session mismatch"
      logging.error(error_message)
      raise ValueError(error_message)
    
    if access_token.family_id != refresh_token.family_id:
      error_message = f"Error: Token pair mismatch"
      logging.error(error_message)
      raise ValueError(error_message)
    ...
    if db_user is None:
      error_message = f"Error: User not found"
      logging.error(error_message)
      raise ValueError(error_message)

    new_token = self.create_auth_tokens(TokenAudience(id=access_token.user_id, uuid=db_user.uuid))
