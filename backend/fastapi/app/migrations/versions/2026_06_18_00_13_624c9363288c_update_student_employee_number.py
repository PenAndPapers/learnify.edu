...
def upgrade() -> None:
  ...
  op.execute(
    "ALTER TABLE employees RENAME CONSTRAINT employees_employee_number_key TO employees_employee_id_key"
  )

  # 2. Rename student column and its unique constraint
  op.alter_column("students", "student_number", new_column_name="student_id")
  op.execute(
    "ALTER TABLE students RENAME CONSTRAINT students_student_number_key TO students_student_id_key"
  )
...
def downgrade() -> None:
  ...
  op.execute(
    "ALTER TABLE students RENAME CONSTRAINT students_student_id_key TO students_student_number_key"
  )
  op.alter_column("students", "student_id", new_column_name="student_number")

  # 2. Reverse employee rename
  op.execute(
    "ALTER TABLE employees RENAME CONSTRAINT employees_employee_id_key TO employees_employee_number_key"
  )
